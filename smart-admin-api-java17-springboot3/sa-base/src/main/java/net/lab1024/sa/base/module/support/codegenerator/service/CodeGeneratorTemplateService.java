...
public class CodeGeneratorTemplateService {
    ...
    public String generate(String tableName, String file, CodeGeneratorConfigEntity codeGeneratorConfigEntity) {
        ...
        //主键字段名称和java类型
        Optional<CodeField> primaryKeyOptional = fields.stream()
                .filter(e -> Boolean.TRUE.equals(e.getPrimaryKeyFlag()))
                .findFirst();

        if (primaryKeyOptional.isPresent()) {
            CodeField primaryKeycodeField = primaryKeyOptional.get();
            ...
            // 没有找到主键，使用默认值
            log.warn("⚠️ 表 {} 的配置中没有主键字段，使用默认值", tableName);
            variablesMap.put("primaryKeyJavaType", "Long");
            variablesMap.put("primaryKeyFieldName", "id");
            variablesMap.put("primaryKeyColumnName", "id");
        }
